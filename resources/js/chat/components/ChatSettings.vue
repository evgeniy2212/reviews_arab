    ...
    <div>
        ...
        <div class="chat__contact" :class="contact.status === 1 ? 'is-online' : ''">
            ...
            <span class="chat__status"
                ...
                {{ contact.status === 1 ? 'متصل' : 'خارج الخدمة' }}
            </span>
        </div>
        <div class="chat__row">
    ...
    watch:{
        ...
        linkSentSuccess(newVal){
            ...
            if(this.showModal){
                if(newVal === true){
                    $('.successMessageContent').remove();
                    $('#successMessageContent').text('تم إرسال الرابط!');
                    $('#successMessageContent').removeAttr('hidden');
                    $('#successMessage').modal();
                } else {
                    $('#errorMessageContent').text('حاول إرسال الرابط مرة أخرى!');
